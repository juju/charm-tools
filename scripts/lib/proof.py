...
def check_relation_hooks(relations, subordinate):
    ...
    for r in relations.items():
        ...
        has_one = has_one or check_hook(
            r + '-relation-changed', required=False)
        has_one = has_one or check_hook(
            r + '-relation-departed', required=False)
        has_one = has_one or check_hook(
            r + '-relation-joined', required=False)
        has_one = has_one or check_hook(
            r + '-relation-broken', required=False)
...
try:
    ...
    if charm['name'] != charm_basename:
        warn("metadata name (%s) must match directory name (%s) exactly for "
             "local deployment." % (charm['name'], charm_basename))
    ...
    if 'maintainer' not in charm:
        ...
        for maintainer in maintainers:
            ...
            if formatted != maintainer:
                warn("Maintainer address should contain a real-name and email"
                     " only. [%s]" % (formatted))
